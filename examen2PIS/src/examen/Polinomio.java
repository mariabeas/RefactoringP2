...
public class Polinomio {
	...
	public String routh(int poli1){
		...
		if(criticamenteEstable){
			solucion+="Sistema cr�ticamente estable -->\nFila cero\nNo hay ra�ces positivas";
		}else if(sistemaInestable){
			solucion+="Sistema inestable --> dos ra�ces positivas (hay dos cambios de signo en primera columna)";
		}else{
			if(!salir){
			solucion+="Sistema estable --> no hay ra�ces en semiplano derecho";
			}else{
			solucion+="Degeneraci�n en el C�lculo --> Primer elemento de fila es cero";
			}
		}
		}
		return solucion;
